...
- **dependencies**:
  - `findata-plugins/tushare_trade_cal` with requirement `{exchanges: ["SSE", "SZSE"], timerange}` for trading-day pruning
  - `findata-plugins/tushare_index_basic` with requirement `{indexes}` for local selector validation and exact
    reference metadata
