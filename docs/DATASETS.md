...
Each dataset entry defines its provider, logical Arrow schema, keys, observation domain, maintenance universe, publication timing, missing-data policy, dependencies, operations and operand semantics, storage, coverage, and status fields. Machine-readable schemas created during implementation must express these contracts without changing them.
...
- `update` is always parameterless. A one-time `complete` or `refresh` never changes a configured maintenance universe.
- Built-in `complete` and `refresh` operations declare their fully normalized operands as a stable coalescing identity. `update` never coalesces because its target depends on submission time, publication state, and the maintenance-universe revision.
...
- **maintenance universe**: intrinsic; exactly `SSE` and `SZSE`
...
- **maintenance universe**: intrinsic; every update fetches the complete snapshot
...
| field | Arrow type | nullable | meaning |
| --- | --- | --- | --- |
| `index_code` | `utf8` | no | canonical Tushare index code |
...
- **aliases**: v1 declares `CSI300` as `000300.SH`; unknown aliases fail validation
- **observation domain**: every calendar month, represented by `[month_start, next_month_start)`
- **maintenance universe**: configured and initially empty; entries are canonical index codes or declared aliases
...
- **dependencies**: none
- **dependency fulfillment**: `{indexes, timerange}` requires both fields, canonicalizes aliases, expands the half-open range to intersecting calendar months, and maps missing months to `complete(indexes=..., timerange=...)`
- **operations**:
  - `update()` — extend every configured index from its coverage end through the current month; a newly selected index begins at the current month, and an empty universe is rejected
  - `complete(indexes, timerange)` — fetch every intersecting historical or current calendar month for the requested indexes while preserving continuous monthly coverage
...
The constituent-set resolver queries `effective_month`. `ALIAS@YYYYMM` requires and selects that month. `ALIAS@latest` requires the month containing the consuming operation's latest due date and selects it; it never silently substitutes an older covered month. Bare `ALIAS` requires every month intersecting the consuming operation's range and returns their constituent union. An inside-window month that remains unavailable after dependency fulfillment therefore produces an explicit coverage failure rather than stale membership.
...
- **maintenance universe**: configured and initially empty; entries are canonical symbols or constituent-set selectors
  - `CSI300@latest` resolves the membership month containing each update's latest due trading date and requires that month to be covered
  - bare `CSI300` in `complete` resolves the historical union over that operation's range
...
- **operations**:
  - `update()` — resolve the configured universe for the latest due trading date and extend those symbols through the next civil-date endpoint; a newly selected symbol begins at that latest due date
