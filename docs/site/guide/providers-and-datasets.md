...
```bash
findata dataset ls
findata dataset describe tushare_daily_basic   # schema, capabilities, settings, storage
findata dataset operations tushare_daily_basic
findata dataset operation tushare_daily_basic complete   # operand schema and per-operand help
findata dataset status tushare_daily_basic     # or: dataset status --all
```
...
| dataset | contents |
| --- | --- |
| `tushare_trade_cal` | exchange trade calendars (SSE, SZSE) |
| `tushare_stock_basic` | listed-stock master data |
| `tushare_index_basic` | index metadata |
| `tushare_index_weight` | monthly index constituent weights |
| `tushare_daily_basic` | daily valuation metrics (PE, PB, turnover, …) |
...
```bash
findata dataset update   tushare_daily_basic                      # parameterless, uses settings
findata dataset complete tushare_daily_basic --symbols 600000.SH \
    --from 2026-01-01 --to 2026-07-01                             # explicit backfill
findata dataset refresh  tushare_daily_basic --symbols 600000.SH \
    --from 2026-06-01 --to 2026-07-01                             # refetch inside coverage
findata dataset reset    tushare_daily_basic --yes                # start over, keep settings
```
...
`tushare_index_basic`. The bare reference in `complete` means the historical constituent
