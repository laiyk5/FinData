...
def daily_basic_plugin() -> "DatasetPlugin":
    ...
    from findata_plugins.tushare.plugins.datasets.stock.daily_basic.operations import (
    ...
    return DatasetPlugin(
        ...
        settings={
            "dataset.findata-plugins/tushare_daily_basic.update_symbols": SettingSpec(
                ...
                help=(
                    "Direct securities and Tushare constituent selectors maintained by update; "
                    "defaults to the symbols already covered by this dataset."
                ),
                default=["stored"],
            )
        },
        schedule=("40 17 * * 1-5", "Asia/Shanghai"),
        family=("tushare", "stock"),
    )
