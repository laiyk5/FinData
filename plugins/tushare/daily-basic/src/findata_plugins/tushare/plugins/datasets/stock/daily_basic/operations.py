...
class DailyBasicDatasetService(TushareDatasetService):
    ...
    def _daily_basic(self, operation: str, operands: dict[str, Any]) -> str:
        if operation == "update":
            ...
            if not selectors:
                self._log("no stored symbols; daily_basic update has no work")
                return None
        ...
        if selectors == ["all"]:
            ...
            self._log(f"plan: entire market over {requested.start.isoformat()}..{requested.end.isoformat()}; request shape: full-market per date")
            publication = self._daily_basic_all_by_date(spec=self.spec, requested=requested, existing_coverage=existing_coverage)
            return publication or self.loader.dataset(self.spec.name).publication_id
    ...
    def _daily_basic_all_by_date(
        ...
        *,
        ...
        """Fetch and commit each available symbol from the provider's daily snapshot."""
        try:
            calendar = self.loader.dataset("findata-plugins/tushare_trade_cal").query(
                keys=["SSE", "SZSE"],
                time_range=(requested.start, requested.end),
                columns=["cal_date"],
                filters=[("is_open", "=", True)],
                require_coverage=True,
            )
        except DataLoaderError as exc:
            raise OperandError("trade calendar coverage is required to fetch the entire market") from exc
        open_dates = sorted(set(calendar.column("cal_date").to_pylist()))
        ...
        for completed, target in enumerate(open_dates, start=1):
            ...
            if table.num_rows >= row_limit:
                raise OperandError(
                    f"daily_basic full-market response reached the declared {row_limit}-row limit "
                    f"on {target.isoformat()}; cannot safely fetch all symbols"
                )
...
_OPERATION_HELP: dict[str, str] = {
    "update": "Extend the symbols already covered by this dataset through the latest due "
    ...
    return {
        ...
        "properties": {
            "symbols": {
                ...
                "items": {"type": "string"},
                "minItems": 1,
                "help": "Use all to fetch the entire market for every open date in the range.",
...
def plan_operation(
    ...
    *,
    ...
    if operation == "update":
        strategy = "stored-coverage update"
    else:
