...
from findata_plugins.tushare.shared.publication import PublicationWindow, monthly_window
...
class IndexWeightDatasetService(TushareDatasetService):
    ...
    def _index_weight(self, operation: str, operands: dict[str, Any]) -> str:
        if operation == "update":
            ...
            references = self._update_setting(self.spec.name) or ["stored"]
            if "stored" in references:
                references = [
                    *[reference for reference in references if reference != "stored"],
                    *(f"tushare:{index}" for index in sorted(existing_coverage)),
                ]
            if not references:
                self._log("no stored indexes; index_weight update has no work")
                return None
...
_OPERATION_HELP: dict[str, str] = {
    "update": "Extend the indexes already covered by this dataset through the current month. "
    "Configure update_indexes to maintain a different or broader selector.",
